...
│   ├── ocr/
│   │   ├── router.py            # /ocr, /ocr/stream, /ocr/stats
│   │   ├── pipeline.py          # encode + ocr_one_page
│   │   ├── pdf_loader.py        # poppler/pdf2image (iter + parallel)
│   │   ├── runtime.py           # shared ThreadPoolExecutor + semaphore
...
│
├── tools/
│   ├── bench_concurrent.py      # benchmark song song dùng aiohttp (chạy trên server)
│   └── bench_local.py           # benchmark dùng urllib từ máy local (không cần aiohttp)
│
├── docs/
...
> **Lưu ý hiệu năng**: nên trỏ UI vào URL cloudflare tunnel thay vì IP trực tiếp.
> Browser giới hạn 6 TCP connection/origin với HTTP/1.1 — chỉ 6 stream OCR song
> song. Cloudflare tunnel dùng HTTP/2 multiplexing → 20+ stream trên 1 kết nối →
> vLLM nhận đủ batch → **0.29 s/trang** thay vì 0.47 s/trang qua IP thường.
...
| GET  | `/ocr/stats` | — | `{requests, pages, failed, inflight_pages, active_streams, peak_streams}` |
...
2. `pdf_loader.iter_pdf_pages` render PDF → ảnh theo từng chunk (poppler, DPI=300);
   submit trang lên vLLM ngay khi render xong từng chunk.
...
4. Keep-alive SSE comment `: ping` gửi mỗi 15 giây khi đang chờ vLLM — ngăn
   cloudflare/proxy cắt connection idle.
5. Stream SSE từng trang về UI.
...
| 0.47 | **Pool OCR dùng chung + `MAX_NUM_SEQS=512`** — vLLM gom gấp đôi sequence mỗi batch decode; vẫn 0.47 khi UI dùng IP thẳng (HTTP/1.1 → 6 stream) |
| **0.29** | **HTTP/2 qua cloudflare tunnel** — multiplexing 20+ SSE stream trên 1 kết nối → vLLM nhận 130+ trang đồng thời; KV prefix cache ~85% hit (page_num đặt cuối prompt). File 221 trang: ~64s |

→ Từ 3–5s xuống **~0.29 s/trang** — nhanh hơn ~10–15×.
...
- **Keep-alive SSE**: server gửi comment `: ping` mỗi 15 giây — cloudflare/nginx
  không cắt connection idle giữa các batch trang.
- **Observability**: `GET /ocr/stats` (counter requests/pages/failed/inflight_pages +
  active_streams/peak_streams) + log có request-id ở mọi mốc start/done/cancel.
...
| `MAX_UPLOAD_MB` | `200` | Chặn file upload quá lớn (`413`) |
...
- Hạ `GPU_MEMORY_UTILIZATION=0.90` nếu OOM.
- `CUDA_VISIBLE_DEVICES=0,1` đúng index GPU.
- Xem log: `journalctl -u vllm@$USER.service -f`.

**OCR chậm hơn mong đợi (~0.47 s/trang thay vì ~0.29 s/trang)**
- Kiểm tra UI đang dùng HTTP/1.1 (IP thẳng) hay HTTP/2 (cloudflare tunnel).
- Browser giới hạn 6 TCP connection/origin với HTTP/1.1 → tối đa 6 SSE stream đồng
  thời → vLLM nhận ít batch hơn → throughput thấp hơn.
- Đổi URL API sang `https://xxx.trycloudflare.com` để dùng HTTP/2 — vLLM sẽ nhận
  130+ request đồng thời, KV prefix cache ~85% hit.

**SSE stream bị ngắt giữa chừng**
- Server gửi `: ping` mỗi 15 giây để giữ connection. Nếu vẫn bị cắt, kiểm tra
  proxy/nginx timeout (`proxy_read_timeout`, `proxy_send_timeout`).
- Cloudflared quick tunnel đôi khi xoay kết nối HTTP/2 → stream SSE bị giết; UI
  tự retry từ trang đã nhận — không phải lỗi server.
